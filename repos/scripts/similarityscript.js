...
(function(ceres)
{
    ...
    {
        if (!window.get('hd')) window.set('hd', 'block', { 'max-age': 7200 });
        if (!window.get('pn')) window.set('pn', 'disabled', { 'max-age': 7200 });

        if (window.get('hd') == 'none') document.getElementById('site-header-display').style.display = 'none';
        if (window.get('pn') == 'enabled') setPinState(document.getElementById('pin-navbar'), 'enabled');
    }
    ...
    function resetDisplayState(attribute)
    {
        ...
        window.set('hd', attribute, { 'max-age': 7200 });
    }
    ...
    function setPinState(el, attribute)
    {
        ...
        window.set('pn', attribute, { 'max-age': 7200 });
    }
